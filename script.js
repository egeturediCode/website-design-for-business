...
addEventListener('DOMContentLoaded', () => {
    ...
    const divs = document.querySelectorAll('.our-services .item .content');
    if (!divs || divs.length === 0) return;

    divs.forEach(div => div.classList.add('no-hover'));

    setTimeout(() => {
        divs.forEach(div => div.classList.remove('no-hover'));
    }, 800);
});
});
